...
class AppStore extends Component {
  ...
  render() {
    ...
    const i = searchInput.toLowerCase()
    list = list.filter(eachItem => eachItem.appName.toLowerCase().includes(i))
    return (
      <div className="bgContainer">
        <h1 className="heading">App Store</h1>
        <div className="inputDiv">
          <input type="search" onChange={this.changeElement} />
        </div>
        ...
        <ul className="cardsList">
          {list.map(eachItem => (
            <App item={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
